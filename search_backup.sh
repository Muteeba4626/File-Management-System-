...
while getopts ":d:ir" opt; do 
  case $opt in 
    ...
    r) DO_REPORT=true ;; 
    \?) echo "Invalid option: -$OPTARG" >&2; exit 0;;             # exit with zero if no choice    
  esac 
done 
shift $((OPTIND -1)) 
...
if [[ "$ans_rsync" =~ ^[Yy]$ ]]; then 
  ...
  if [ "$DO_INCREMENTAL" = true ] && [ -d "$PREV_FOLDER" ] && [ "$PREV_FOLDER" != "$BACKUP_FOLDER" ]; then 
    rsync -a --files-from="$TMP" --link-dest="../$(basename "$PREV_FOLDER")" "$SRC_DIR/" "$BACKUP_FOLDER/"    #link to add with previous backup 
  else 
    rsync -a --files-from="$TMP" "$SRC_DIR/" "$BACKUP_FOLDER/"                #link if no previous backup 
  fi 
  printf "\n" 
